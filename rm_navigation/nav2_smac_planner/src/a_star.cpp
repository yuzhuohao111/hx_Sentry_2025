...
void AStarAlgorithm<NodeT>::setGoal(
  const unsigned int & mx,  // 目标节点的x坐标
  const unsigned int & my,  // 目标节点的y坐标
  const unsigned int & dim_3)  // 目标节点的第三维度坐标（如果有）
{
  // 将目标节点添加到图中，并返回对应的节点指针
  ...
  typename NodeT::Coordinates goal_coords(
    ...
    static_cast<float>(dim_3));  // 将第三维度坐标转换为浮点数

  // 如果不需要缓存障碍物启发式信息，或者目标坐标与当前缓存的目标坐标不一致
  ...
  const float & tolerance)
{
  ...
  // 给定一个索引，如果它是无碰撞且有效的，则返回一个节点指针引用
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    // 1) 从开放集中选择Nbest，使得f(Nbest)最小，并将其从队列中移除
    current_node = getNextNode();

    // 我们允许节点多次入队，以防更短的路径导致它被访问，但我们只能访问一次
      ...
      neighbor_iterator != neighbors.end(); ++neighbor_iterator)
    {
      ...
      // 4.2) 如果这是比之前更低的成本，我们将其设置为新的成本和新路径
