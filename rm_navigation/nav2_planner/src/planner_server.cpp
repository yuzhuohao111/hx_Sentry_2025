...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  // 初始化区域配置 - 根据实际需求修改这些坐标和模式
  //home r
  regions_ = {
    {
      // X坐标序列
      {9.0, 11.0, 11.0, 9.0, 9.0},
      // Y坐标序列
      {-5.0,-5.0, -6.0, -6.0,-5.0},
      // 模式值
      2
    ...
    {
      // X坐标序列
      {5.0, 5.2, 6.0, 7.5, 7.5, 6.0, 5.0},
      // Y坐标序列
      {-2.0,-2.5, -2.5, -1.5,-1.0, -1.5, -2.0},
      // 模式值
      1
    },
    {
      // X坐标序列
      {11.7, 13.7, 13.6, 11.6, 11.7},
      // Y坐标序列
      {3.1, 3.9, 4.5, 4.0, 3.1},
      // 模式值
      3
  ...
  // };
  // 创建区域可视化发布器
  regions_publisher_ = create_publisher<visualization_msgs::msg::MarkerArray>(
    "regions_visualization", 10);

  // Initialize pubs & subs
  plan_publisher_ = create_publisher<nav_msgs::msg::Path>("plan", 1);
  mode_publisher_ = create_publisher<rm_decision_interfaces::msg::Mode>("/mode_nav", 10);
...
geometry_msgs::msg::Point PlannerServer::createPoint(double x, double y, double z)
{
...
// 判断点是否在多边形内（射线法）
bool PlannerServer::isPointInPolygon(
  const geometry_msgs::msg::Point& point, 
  const std::vector<double>& x_coords,
  const std::vector<double>& y_coords)
{
  if (x_coords.size() != y_coords.size() || x_coords.size() < 3) {
    return false;
  }
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    double x2 = x_coords[j];
    double y2 = y_coords[j];
    
    // 检查点是否在线段左侧
    if (((y1 <= point.y) && (y2 > point.y)) || 
        ((y1 > point.y) && (y2 <= point.y))) {
      double intersect_x = (point.y - y1) * (x2 - x1) / (y2 - y1) + x1;
...
int PlannerServer::checkPathThroughRegions(const nav_msgs::msg::Path & path)
{
  // 可视化区域
  visualizeRegions();
  ...
  for (const auto& pose : path.poses) {
    for (const auto& region : regions_) {
      if (isPointInPolygon(pose.pose.position, region.x_coords, region.y_coords)) {
        RCLCPP_INFO(
          get_logger(), 
          "Path passes through region (mode %d) at (%.2f, %.2f)", 
          region.mode, pose.pose.position.x, pose.pose.position.y);
  ...
  // RCLCPP_INFO(get_logger(), "Path does not pass through any defined region");
  return 0; // 没有区域匹配
}

void PlannerServer::visualizeRegions()
{
  if (!regions_publisher_->is_activated()) {
    return;
  }
  ...
  for (const auto& region : regions_) {
    ...
    // 添加多边形顶点
    for (size_t i = 0; i < region.x_coords.size(); ++i) {
      geometry_msgs::msg::Point p;
      p.x = region.x_coords[i];
      p.y = region.y_coords[i];
      p.z = 0.05;  // 稍微抬高避免与地面重叠
      marker.points.push_back(p);
    }
    
    // 闭合多边形
    if (!region.x_coords.empty()) {
      geometry_msgs::msg::Point first;
      first.x = region.x_coords[0];
      first.y = region.y_coords[0];
      first.z = 0.05;
    ...
    for (size_t i = 0; i < region.x_coords.size(); ++i) {
      center_x += region.x_coords[i];
      center_y += region.y_coords[i];
    }
    center_x /= region.x_coords.size();
    center_y /= region.x_coords.size();
    ...
    text_marker.text = "Mode " + std::to_string(region.mode);
  ...
  regions_publisher_->publish(std::move(marker_array));
}
...
PlannerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  regions_publisher_->on_activate(); 
...
PlannerServer::computePlanThroughPoses()
{
  ...
  try {
    ...
    // ====== 新增：检查路径是否经过矩形区域 ======
    int region_mode = checkPathThroughRegions(result->path);
    publishMode(region_mode); // 发布对应的模式
...
// 发布模式消息
void PlannerServer::publishMode(int mode_value)
{
  auto mode_msg = std::make_unique<rm_decision_interfaces::msg::Mode>();
  mode_msg->mode = mode_value;
  mode_publisher_->publish(std::move(mode_msg));
  // RCLCPP_INFO(get_logger(), "111111111111Published mode: %d", mode_value);
}
